...
const heroStripAr = [
  { num: '01', label: 'مكالمة التوافق' },
  { num: '02', label: 'التشخيص' },
  { num: '03', label: 'تصميم المنظومة' },
]
...
const whatItMeansAr = [
  'DigitalIQ ما بتبدأش بباكجات.',
  'بتبدأ بفهم الوضع الحقيقي للبيزنس.',
  'ده معناه النظر بعيدًا عن نسخة الداشبورد لما بيحصل فعلًا تحت التسويق.',
  'ده مش طقس مبيعات. ده بداية تقييم جاد.',
]

const stepOutcomesEn: Record<string, string[]> = {
  'STEP 01': [
...
const stepOutcomesAr: Record<string, string[]> = {
  'STEP 01': [
    'صورة واضحة عن أين البراند دلوقتي',
    'تقييم صادق هل DigitalIQ الشريك المناسب',
    'خطوة تالية محددة لو في توافق',
  ],
  'STEP 02': [
    'خريطة لإيه اللي شغال وإيه اللي مش شغال',
    'الأسباب الحقيقية لبطء النمو أو توقفه',
    'قائمة مرتبة لإيه اللي محتاج اهتمام أول',
  ],
  'STEP 03': [
    'نموذج نمو مخصص مبني حول البراند',
    'تسلسل محدد: أيه المحركات، بأيه ترتيب',
    'سكوب مبني على واقع البيزنس مش باكجات',
  ],
}
...
export default function StartNowPage() {
  ...
  const heroStrip = isAr ? heroStripAr : heroStripEn
  const whatItMeansStatements = isAr ? whatItMeansAr : whatItMeansEn
  const stepOutcomes = isAr ? stepOutcomesAr : stepOutcomesEn
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.phone.trim()) newErrors.phone = isAr ? 'رقم التليفون مطلوب.' : 'Phone number is required.'
        ...
        <div className="container-site relative z-10">
          ...
          >
            <motion.div variants={staggerItem} className="mb-6">
              <span className="text-label-md font-satoshi uppercase tracking-widest text-accent">{isAr ? 'ابدأ دلوقتي' : 'Start Now'}</span>
            </motion.div>
            <motion.h1
        ...
        <div className="container-site">
          ...
          >
            <motion.h2 variants={staggerItem} className="text-heading-xl font-satoshi text-text-primary mb-10">
              {isAr ? 'ابدأ دلوقتي معناها إيه فعلًا' : 'What Start Now Actually Means'}
            </motion.h2>
        ...
        <div className="container-site">
          <div className="mb-14">
            <h2 className="text-heading-xl font-satoshi text-text-primary">
              {isAr ? 'بداية منظمة. بدون تخمين.' : 'A Structured Beginning. No Guesswork.'}
            </h2>
          </div>
        ...
        <div className="container-site">
          <div className="max-w-[680px]">
            <h2 className="text-heading-xl font-satoshi text-text-primary mb-6">
              {isAr ? 'ليه العملية دي موجودة' : 'Why This Process Exists'}
            </h2>
        ...
        <div className="container-site">
          ...
          >
            ...
            <motion.div variants={staggerItem} className="mb-14">
              <span className="block text-label-md font-satoshi uppercase tracking-widest text-text-muted mb-5">{isAr ? 'تواصل معنا' : 'Get In Touch'}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-12 lg:gap-16 items-start">
              ...
              <motion.div variants={staggerItem} className="space-y-8">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-label-md text-text-muted uppercase tracking-widest font-satoshi mb-1">{isAr ? 'إيميل' : 'Email'}</p>
                    <Link
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-label-md text-text-muted uppercase tracking-widest font-satoshi mb-1">{isAr ? 'تليفون' : 'Phone'}</p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-label-md text-text-muted uppercase tracking-widest font-satoshi mb-1">{isAr ? 'العنوان' : 'Address'}</p>
                    <p className="text-body-md text-text-secondary leading-snug">
                      {isAr ? <>شارع الأمل 3أ،<br />الشيخ زايد، الجيزة</> : <>3A El-Amal Street,<br />El Shiekh Zayed, Giza</>}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <span className="text-body-md font-satoshi">{isAr ? 'تكلم على واتساب' : 'Chat on WhatsApp'}</span>
                </Link>

                {/* Social links */}
                <div>
                  <p className="text-label-md text-text-muted uppercase tracking-widest font-satoshi mb-4">{isAr ? 'تابعونا' : 'Follow Us'}</p>
                ...
                <div className="relative bg-surface-02 border border-surface-border rounded-2xl p-8 md:p-10 shadow-elev-3">
                  ...
                  {submitted ? (
                    ...
                    >
                      ...
                      <div>
                        <h3 className="text-heading-lg font-satoshi text-text-primary mb-2">{isAr ? 'وصلتنا رسالتك.' : 'Message received.'}</h3>
                        <p className="text-body-md text-text-secondary">{isAr ? 'هنتواصل معك خلال 24 ساعة لتحديد مكالمة التوافق.' : 'We will be in touch within 24 hours to schedule the Alignment Call.'}</p>
                      </div>
                    </motion.div>
                  ) : (
                    <form onSubmit={handleSubmit} noValidate className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                          <label htmlFor="name" className="block text-label-md font-satoshi text-text-secondary uppercase tracking-widest mb-2">
                            {isAr ? 'الاسم' : 'Full Name'} <span className="text-accent">*</span>
                          </label>
                          <input
                            id="name"
                            name="name"
                        ...
                        <div>
                          <label htmlFor="phone" className="block text-label-md font-satoshi text-text-secondary uppercase tracking-widest mb-2">
                            {isAr ? 'التليفون' : 'Phone'} <span className="text-accent">*</span>
                          </label>
                          <input
                            id="phone"
                            name="phone"
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          value={form.message}
                          onChange={handleChange}
                          placeholder={isAr ? 'احكيلنا عن البراند وأين تحاول توصل.' : 'Tell us about your brand and where you are trying to go.'}
                      ...
                      >
                        {isAr ? 'ابعت الرسالة' : 'Send Message'}
                      ...
                      <p className="text-label-md text-text-muted text-center">
                        {isAr ? 'بنرد خلال 24 ساعة. بدون بيتش. بدون ضغط.' : 'We respond within 24 hours. No pitch, no pressure.'}
                      </p>
                    </form>
                  )}
                </div>
